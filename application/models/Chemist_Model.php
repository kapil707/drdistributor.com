...
if ($items != '') {
	...
	public function insert_value_on_session($user_session='',$user_fname='',$user_code='',$user_altercode='',$user_type='',$user_password='',$user_division='',$user_compcode='',$user_image='',$user_nrx='') 
	{		
...
$items.= <<<EOD
{"id":"{$id}","funtype":"{$funtype}","itemid":"{$itemid}","division":"{$division}","image":"{$image}","compname":"{$compname}","compid":"{$compid}","web_action":"{$web_action}"},
EOD;
		}
if ($items != '') {
	$items = substr($items, 0, -1);
	...
	public function top_flash2()
	{
		...
		foreach ($query as $row)
		{
			...
			$itemid	    =	$row->itemid;
			$division	=	$row->division;
			$image 		= 	constant('img_url_site')."uploads/manage_slider2/photo/main/".$row->image;
$items.= <<<EOD
...
if ($items != '') {
	...
	public function must_buy_medicines_json_50($session_yes_no="no",$category_id)
	{
		...
		$date = date("Y-m-d");
		$sameid = "";
		$query = $this->db->query("select DISTINCT i_code, COUNT(*) as `quantity` FROM tbl_order where date='$date' GROUP BY i_code,item_name HAVING COUNT(*) > 1 order by quantity desc,RAND() limit 12")->result();
		foreach ($query as $row)
		{
			$sameid.=$row->i_code.",";
		}
...
if ($items != '') {
	...
	public function medicine_new_scheme_json_50($session_yes_no="no",$category_id)
	{
		...
		$items = "";
		$sameid = "";
		$query = $db2->query("select DISTINCT i_code from tbl_medicine_compare_final where type='margin' ORDER BY RAND() limit 12")->result();
		foreach ($query as $row)
		{
			$sameid.=$row->i_code.",";
		}
...
if ($items != '') {
	...
	public function stock_now_available()
	{
		$items = "";
		$sameid = "";
		$query = $this->db->query("SELECT tbl_medicine.i_code FROM `tbl_stock_low` inner join tbl_medicine on tbl_medicine.i_code=tbl_stock_low.i_code where tbl_medicine.batchqty!='0' order by RAND() LIMIT 15")->result();
		foreach ($query as $row)
		{
			$sameid.=$row->i_code.",";
		}
...
if ($items != '') {
	...
	public function stock_now_available_json_50($session_yes_no="no",$category_id)
	{
		...
		$items = "";
		$sameid = "";
		//$query = $this->db->query("SELECT tbl_medicine.i_code FROM `tbl_stock_low` inner join tbl_medicine on tbl_medicine.i_code=tbl_stock_low.i_code where tbl_medicine.batchqty!='0' order by RAND() LIMIT 25")->result();
...
if ($items != '') {
	...
	public function user_top_search_items_json_50($session_yes_no,$category_id,$user_type,$user_altercode,$salesman_id)
	{
		$items = "";
		$sameid = "";
		$query = $this->db->query("select item_code from tbl_top_search where user_type='$user_type' and user_altercode='$user_altercode' and salesman_id='$salesman_id' limit 12")->result();
		foreach ($query as $row)
		{
			if(!empty($row->item_code))
			{
...
$items.= <<<EOD
{"item_code":"{$code}","item_image":"{$image}","item_name":"{$name}","item_packing":"{$packing}","item_company":"{$company}","quantity":"{$quantity}","item_stock":"{$stock}","item_mrp":"{$mrp}","item_ptr":"{$ptr}","item_price":"{$price}","item_margin":"{$margin}","item_featured":"{$featured}"},
EOD;
				}
			}
		}
...
$items.= <<<EOD
{"item_code":"{$code}","item_image":"{$image}","item_name":"{$name}","item_packing":"{$packing}","item_company":"{$company}","item_quantity":"{$quantity}","stock":"{$stock}","item_mrp":"{$mrp}","item_ptr":"{$ptr}","item_price":"{$price}","item_margin":"{$margin}","item_featured":"{$featured}"},
EOD;
					}
				}
			}
			...
			if(file_exists($excelFile_delete)=="1")
			{
				...
				foreach ($objPHPExcel->getWorksheetIterator() as $worksheet)
				{
					...
					for ($row=$headername; $row<=$highestRow; $row++)
					{
						...
						$item_remarks1 = $worksheet->getCell($remarks_r.$row)->getValue();
						//=="ITEM DELETE"
						...
						/* if (stripos($item_remarks1, "OLD QTY") !== false) {
							//echo "hello";
							$x = str_replace(" ","",$item_remarks1);
							$x = str_replace("OLDQTY:","OLDQTY,",$x);
							$x = str_replace("NEWQTY:","NEWQTY,",$x);
							$x = str_replace("BATCH","BATCH,",$x);
							$str_arr = explode(",",$x);
							$min = (int)$str_arr[1];
							$max = (int)$str_arr[2];
							if($min>$max){
...
if ($edit_items != ''){
	$edit_items = substr($edit_items, 0, -1);
}
$header_title= <<<EOD
