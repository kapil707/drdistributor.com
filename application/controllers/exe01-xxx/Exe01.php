...
INSERT INTO tbl_order (online_id,order_id,item_code,quantity,chemist_id,user_type,salesman_id,temp_rec,remarks,sale_rate,order_status,date,time,gstvno,new_temp_rec,total_line) VALUES <?= $items;?>;<?= $row->order_id ?>
	...
	public function medicine_upload()
	{
		...
		foreach($items as $ks)
		{
			...
			$this->db->query("insert into tbl_medicine_essol (i_code,item_code,item_name,title,packing,expiry,batch_no,batchqty,salescm1,salescm2,sale_rate,mrp,final_price,margin,costrate,compcode,comp_altercode,company_name,company_full_name,division,qscm,hscm,misc_settings,item_date,itemcat,gstper,itemjoinid,present,featured,discount,image1,image2,image3,image4,title2,description,note,category,status,time) values ".$qry);
	...
	public function chemist_upload()
	{
		...
		{
			$qry = base64_decode($ks["qry"]);
			$this->db->query("insert into tbl_chemist_essol (code,altercode,groupcode,name,type,trimname,address,address1,address2,address3,telephone,telephone1,mobile,email,gstno,status,statecode,invexport,slcd,narcolicence) values ".$qry);
			echo $ks["thisid"];
		}
	}
