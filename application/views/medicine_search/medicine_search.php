		...
		success: function(data){
			...
			$.each(data.items, function(i,item){
				if (item)
				{
					...
					$(".get_medicine_favourite_div").append('<a href="javascript:void(0)" onClick="medicine_details_funcation('+item_code+')" style="text-decoration: none;"><div class="main_theme_li_bg"><div class="medicine_cart_small_div3"><img src="'+item_image+'" style="width: 100%;cursor: pointer;" class="all_item_image" onclick="medicine_details_funcation('+item_code+')" onerror="setDefaultImage(this);"></div><div class="medicine_cart_small_div4"><div class="text-capitalize all_item_name">'+item_name+'</div><div class="text-left all_order_quantity">Last order quantity : '+item_quantity+'</div></div></div></a>'+div_all_data);
				}
			});
		},
		timeout: 10000
