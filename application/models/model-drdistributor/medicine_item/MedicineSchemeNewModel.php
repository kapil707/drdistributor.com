...
class MedicineSchemeNewModel extends CI_Model  
{
	...
	public function __construct(){
		parent::__construct();
		$this->medicine_db = $this->load->database('default2', TRUE);
	}
	...
	{		
		...
		if($order_by_type=="RAND"){
			$query = $db2->query("select DISTINCT i_code from tbl_medicine_compare_final where type='scheme' ORDER BY RAND() limit 250")->result();
		}else{
			$query = $db2->query("select DISTINCT i_code from tbl_medicine_compare_final where type='scheme' ORDER BY i_code desc limit 250")->result();
		}
