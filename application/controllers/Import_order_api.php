...
class Import_order_api extends CI_Controller {
	...
	public function upload_import_file_api(){
		...
		if(empty($row->id))
		{
			$this->db->query("insert into tbl_import_order_excel_file set headername='$headername',itemname='$itemname',itemqty='$itemqty',itemmrp='$itemmrp',user_altercode='$ChemistId'");
		}
		else
		{
			$this->db->query("update tbl_import_order_excel_file set headername='$headername',itemname='$itemname',itemqty='$itemqty',itemmrp='$itemmrp' where user_altercode='$ChemistId'");
		}
