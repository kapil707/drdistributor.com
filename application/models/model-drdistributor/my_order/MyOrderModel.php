...
class MyOrderModel extends CI_Model  
{
	...
	public function get_my_order_api($user_type="",$user_altercode="",$salesman_id="",$get_record="",$limit=12) {		
		...
		$query = $this->db->query("SELECT order_id,total,gstvno,date,time FROM `tbl_cart_order` WHERE `chemist_id`= '$user_altercode' and salesman_id='$salesman_id' order by order_id desc limit $get_record,$limit")->result();
